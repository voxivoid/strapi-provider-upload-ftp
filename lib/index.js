...
module.exports = {
  ...
  init: (config) => {
    const { host, port, user, password, baseUrl, customPath } = config;
    ...
    return {
        ...
        return new Promise((resolve, reject) => {
          connection.then(() => {
            ftp.list((err, list) => {
              ...
              ftp.append(file.buffer, customPath ? customPath + "/" + fileName : fileName, (err) => {
                if (err) {
                  return reject(err);
                }
      ...
      delete(file) {
        return new Promise((resolve, reject) => {
          connection.then(() => {
            ftp.delete(customPath ? customPath + "/" + file.public_id : file.public_id, (err) => {
              console
              if (err) {
                return reject(err);
              }
